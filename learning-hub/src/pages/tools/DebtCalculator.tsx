...
function pickUI<K extends keyof typeof UI>(key: K, isCA: boolean): (typeof UI)[K]["en"] {
  // en and ru have the same shape on each entry; runtime is a property pick.
  return (isCA ? UI[key].ru : UI[key].en) as (typeof UI)[K]["en"];
}
  ...
  return (
    <>
      <Helmet>
        <title>
          {isCentralAsia
            ? `Бесплатный калькулятор погашения долгов | ${siteConfig.name}`
            : `Free Debt Payoff Calculator - Snowball vs Avalanche | ${siteConfig.name}`}
        </title>
        <meta
          name="description"
          content={
        ...
        <meta
          property="og:url"
          content={`${siteConfig.url}/tools/debt-calculator`}
        />
