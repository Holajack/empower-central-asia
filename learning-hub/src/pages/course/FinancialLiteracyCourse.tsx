  ...
  return (
    <>
      <Helmet>
        <title>
          {isCentralAsia
            ? `Бесплатный 6-недельный курс финансовой грамотности | ${siteConfig.name}`
            : `Free 6-Week Financial Literacy Course | ${siteConfig.name}`}
        </title>
        <meta
          name="description"
          content={
        ...
        <meta
          property="og:url"
          content={`${siteConfig.url}/course/financial-literacy`}
        />
        ...
        <meta name="twitter:title" content={isCentralAsia ? "Бесплатный курс финансовой грамотности" : `Free Financial Literacy Course | ${siteConfig.shortName}`} />
        ...
        <script type="application/ld+json">
          {JSON.stringify(
            generateCourseSchema({
              name: isCentralAsia ? "6-недельный курс финансовой грамотности" : "6-Week Financial Literacy Course",
              description: isCentralAsia
                ? "Бесплатный онлайн-курс: бюджет с нуля, избавление от долгов, накопления, подушка безопасности и финансовые цели."
                : "Free, self-paced financial education covering budgeting, debt elimination, saving strategies, emergency funds, and goal setting.",
              url: absoluteUrl("/course/financial-literacy", language),
              workload: "PT12H",
              weeks: 6,
              level: "Beginner",
              lang: language,
              teaches: ["Personal budgeting", "Zero-based budgeting", "Debt payoff strategies", "Emergency fund planning", "Financial goal setting"],
              syllabus: courseWeeks.map((w) => ({
                name: isCentralAsia ? `Неделя ${w.week}: ${w.title}` : `Week ${w.week}: ${w.title}`,
                description: w.subtitle,
                url: absoluteUrl(`/course/financial-literacy/week-${w.week}`, language),
              })),
            })
          )}
        </script>
          ...
          <div className="container mx-auto px-4 text-center">
            ...
            {hasEmail ? (
              <Link
                to={
                  progress.completedWeeks.length > 0
                    ? `/course/financial-literacy/week-${Math.min(
                ...
                >
                  ...
                  <CardContent className="pt-6 pb-5">
                    ...
                    {status === "locked" ? (
                      ...
                      <Link to={`/course/financial-literacy/week-${week.week}`}>
                        <Button
                          variant="outline"
                          size="sm"
