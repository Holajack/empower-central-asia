...
const LeadershipCourseWeek = () => {
  ...
  if (!isUnlocked) {
    return (
      <CourseWeekGate
        ...
        pageTitle={isCentralAsia ? `Неделя ${weekNum}, День ${currentDay}: ${weekTitle} - Курс лидерства | ${siteConfig.name}` : `Week ${weekNum}, Day ${currentDay}: ${weekTitle} - Leadership Course | ${siteConfig.name}`}
        pageDescription={isCentralAsia ? `${weekSubtitle || weekOverview}. Бесплатный онлайн-курс развития лидерства от ${siteConfig.name}.` : `${weekSubtitle || weekOverview}. Free online leadership development course from ${siteConfig.name}.`}
      >
      <Helmet>
        <title>{isCentralAsia ? `Неделя ${weekNum}, День ${currentDay}: ${weekTitle} - Курс лидерства | ${siteConfig.name}` : `Week ${weekNum}, Day ${currentDay}: ${weekTitle} - Leadership Course | ${siteConfig.name}`}</title>
        <meta name="description" content={isCentralAsia ? `${weekSubtitle || weekOverview}. Бесплатный онлайн-курс развития лидерства от ${siteConfig.name}.` : `${weekSubtitle || weekOverview}. Free online leadership development course from ${siteConfig.name}.`} />
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4 pt-20">
        <div className="text-center max-w-md">
          ...
          <Link to={`/course/leadership-development/week-${weekNum - 1}`}>
            <Button className="bg-[#C9922A] hover:bg-[#C9922A]/90 text-white">
  ...
  function handleSelectDay(week: number, day: number) {
    if (week !== weekNum) {
      window.location.href = `${isCentralAsia ? "/ru" : ""}/course/leadership-development/week-${week}?day=${day}`;
      return;
    }
    navigateDay(day);
  }
  ...
  return (
    ...
    <>
      <Helmet>
        <title>
          {isCentralAsia
            ? `Неделя ${weekNum}, День ${currentDay}: ${weekTitle} - Курс лидерства | ${siteConfig.name}`
            : `Week ${weekNum}, Day ${currentDay}: ${weekTitle} - Leadership Course | ${siteConfig.name}`}
        </title>
        <meta
          name="description"
          content={
            isCentralAsia
              ? `${weekSubtitle || weekOverview}. Бесплатный онлайн-курс развития лидерства от ${siteConfig.name}.`
              : `${weekSubtitle || weekOverview}. Free online leadership development course from ${siteConfig.name}.`
          }
        />
      </Helmet>
